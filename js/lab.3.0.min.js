...
function loadResource(registryEntry){if(registryEntry.type=="script"||registryEntry.type=="module"){var elem=document.createElement("script");elem.setAttribute("src",registryEntry.src);elem.setAttribute("data-requested-with","LABjs");if(registryEntry.type=="module"){elem.setAttribute("type","module");}}
...
var publicAPI={setGlobalDefaults:setGlobalDefaults,setOptions:setOptions,script:script,wait:wait,sandbox:createSandbox,noConflict:noConflict,};return publicAPI;function registerMarkupLinks(){for(var i=0;i<linkElems.length;i++){(function loopScope(elem){if(elem&&/\bpreload\b/i.test(elem.getAttribute("rel"))){var preloadAs=elem.getAttribute("as");if(!preloadAs)return;var href=elem.getAttribute("href");var perfEntries=context.performance.getEntriesByName(href);if(href in registry)return;var registryEntry=new createRegistryEntry(null,href);registryEntry.preloadRequested=true;registry[href]=registryEntry;if(perfEntries.length>0){registryEntry.preloaded=true;}
...
scheduleChainCheck();return chainAPI;}
function addChainResource(resourceType,resourceRecord){if(chain.length==0||!chain[chain.length-1].isGroup||chain[chain.length-1].complete||(chainOpts[optAlwaysPreserveOrder]&&!scriptOrderedAsync)){var groupEntry=new createGroupEntry(scheduleChainCheck);chain.push(groupEntry);}
